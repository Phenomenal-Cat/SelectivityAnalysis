function MF3D_PlotResponses_Expressions(Subject, Date, Channel, CellIndx, Output)
    ...
    for el = 1:numel(Params.Elevations)
        for az = 1:numel(Params.Azimuths)
            ...
            for t = 1:numel(TwinPos)
                ...
                OrientRawMean(exp,az,el,t)      = mean(mean(OrientationSDF{exp,az,el}(:,Tindx)));
                OrientBaseMean(exp,az,el,t)     = mean(mean(OrientationSDF{exp,az,el}(:,BaseIndx)));
                OrientRawSEM{exp,t}(az,el)      = std(std(OrientationSDF{exp,az,el}(:,Tindx)))/sqrt(size(OrientationSDF{exp,az,el},1));
                OrientDiffMat{exp,t}(az,el)     = OrientRawMean(exp,az,el,t)-OrientBaseMean(exp,az,el,t);
                MaxDiffMat(exp,t)               = max(OrientDiffMat{exp,t}(:));
                MinDiffMat(exp,t)               = min(OrientDiffMat{exp,t}(:));
            end

            %========= Plot expression x head oreintation SDF
            axes(Axh(SDFAxIndx(AxIndx)));
            BinSEM{exp,az,el} = std(OrientationSDF{exp,az,el})/sqrt(size(OrientationSDF{exp,az,el}, 1));
            [ha, hb, hc] = shadedplot(HistBins, mean(OrientationSDF{exp,az,el})-BinSEM{exp,az,el}, mean(OrientationSDF{exp,az,el})+BinSEM{exp,az,el}, [1,0.5,0.5]);
            hold on;
            delete([hb, hc]);
            plot(HistBins, mean(OrientationSDF{exp,az,el}), '-r');
...
for exp = 1:numel(Params.Expressions)
    ...
    PixelOffset             = round([HeadImSize(1)/size(OrientDiffMat{exp, t},2), HeadImSize(2)/size(OrientDiffMat{exp, t},1)]/2);
    imh(exp,1)            	= imagesc([PixelOffset(2), HeadImSize(2)-PixelOffset(2)],[PixelOffset(1), HeadImSize(1)-PixelOffset(1)], OrientDiffMat{exp, t}');                                                       
    hold on
    ...
    colormap hot
    box off
    set(gca,'xtick',linspace(PixelOffset(2), HeadImSize(2)-PixelOffset(2), size(OrientDiffMat{exp, t},1)),...
            'xticklabel',Params.Azimuths, ...
            'ytick', linspace(PixelOffset(1), HeadImSize(1)-PixelOffset(1), size(OrientDiffMat{exp, t},2)),...
...
if strcmpi(Output, 'gif')
    giffilename = fullfile(SaveDir, sprintf('ExpressionTimeline_%s_%s_ch%03d_cell%d.gif',Subject, Date, Channel, CellIndx));
    ...
    for t = 1:numel(TwinPos)
        ...
        for exp = 1:numel(Params.Expressions)
            set(imh(exp,1), 'cdata', OrientDiffMat{exp, t}');
        end
        drawnow
        frame = getframe(fh);
        im = frame2im(frame);
...
elseif strcmpi(Output, 'svg')
    print('-painters', '-dsvg', fullfile(SaveDir, sprintf('ExpressionTuning_%s_%s_ch%03d_cell%d.svg',Subject, Date, Channel, CellIndx)))
    export_fig(fullfile(SaveDir, sprintf('ExpressionTuning_%s_%s_ch%03d_cell%d.png',Subject, Date, Channel, CellIndx)), '-png');
end
